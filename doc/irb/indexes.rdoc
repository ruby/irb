...
- {IRB-Specific Commands}[indexes_rdoc.html#label-IRB-Specific+Commands].
...
- <tt>:ECHO</tt>: Whether to print ({echo}[rdoc-ref:IRB@Return-Value+Printing+-28Echoing-29])
  return values;
  initial value: +nil+, which sets +conf.echo+ to +true+.
- <tt>:ECHO_ON_ASSIGNMENT</tt>:
- <tt>:EVAL_HISTORY</tt>:
- <tt>:EXTRA_DOC_DIRS</tt>:
- <tt>:IGNORE_EOF</tt>:
- <tt>:IGNORE_SIGINT</tt>:
- <tt>:INSPECT_MODE</tt>:
- <tt>:IRB_LIB_PATH</tt>:
...
- <tt>:MAIN_CONTEXT</tt>:
- <tt>:MEASURE</tt>:
- <tt>:MEASURE_CALLBACKS</tt>:
- <tt>:MEASURE_PROC</tt>:
- <tt>:PROMPT</tt>:
- <tt>:PROMPT_MODE</tt>:
- <tt>:RC</tt>:
- <tt>:RC_NAME_GENERATOR</tt>:
- <tt>:SAVE_HISTORY</tt>:
- <tt>:SINGLE_IRB</tt>:
- <tt>:USE_AUTOCOMPLETE</tt>:
- <tt>:USE_COLORIZE</tt>:
- <tt>:USE_LOADER</tt>:
- <tt>:USE_TRACER</tt>:
- <tt>:VERBOSE</tt>:
