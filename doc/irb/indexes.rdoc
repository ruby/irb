...
This page has two indexes:

- {Command-Line Options}[indexes_rdoc.html#label-Command-Line+Options].
- {IRB.conf Hash Entries}[indexes_rdoc.html#label-IRB.conf+Entries].

=== Command-Line Options

These are the \IRB command-line options, with links to explanatory text:

- <tt>-d</tt>: Set {$DEBUG}[rdoc-ref:IRB@Debug] and {$VERBOSE}[rdoc-ref:IRB@Verbosity]
  to +true+.
...
- <tt>--autocomplete</tt>: Use {auto-completion}[rdoc-ref:IRB@Auto-Completion].
- <tt>--back-trace-limit _n_</tt>: Set a {backtrace limit}[rdoc-ref:IRB@Backtrace+Limit];
...
- <tt>--context-mode _n_</tt>: Select method to create Binding object
  for new {workspace}[rdoc-ref:IRB@Workspaces]; +n+ in range <tt>0..4</tt>.
...
- <tt>--noautocomplete</tt>: Don't use {auto-completion}[rdoc-ref:IRB@Auto-Completion].
...
- <tt>--noprompt</tt>: Don't print prompts.
- <tt>--noscript</tt>:  Treat the first command-line argument as a normal
  {command-line argument}[rdoc-ref:IRB@Command-Line+Arguments],
...
- <tt>--prompt _mode_</tt>, <tt>--prompt-mode _mode_</tt>:
  Set {prompt and return formats}[rdoc-ref:IRB@IRB+Prompt+and+Return+Formats];
...
- <tt>--script</tt>: Treat the first command-line argument as the path to an
  {initialization script}[rdoc-ref:IRB@Command-Line+Arguments],
...
- <tt>:AT_EXIT</tt>: Array of hooks to call
  {at exit}[rdoc-ref:IRB@Stopping+IRB];
  initial value: <tt>[]</tt>.
- <tt>:AUTO_INDENT</tt>: Whether {automatic indentation}[rdoc-ref:IRB@Automatic+Indentation]
  is enabled; initial value: +true+.
- <tt>:BACK_TRACE_LIMIT</tt>: Sets the {back trace limit}[rdoc-ref:IRB@Back+Trace+Limit];
  initial value: +16+.
...
- <tt>:MAIN_CONTEXT</tt>: The {context}[rdoc-ref:IRB@Configuration] for the main \IRB session;
...
- <tt>:PROMPT</tt>: \Hash of {defined prompts}[rdoc-ref:IRB@IRB+Prompt+and+Return+Formats];
  initial value:
...
- <tt>:PROMPT_MODE</tt>: Name of {current prompt}[rdoc-ref:IRB@IRB+Prompt+and+Return+Formats];
...
- <tt>:SAVE_HISTORY</tt>: Number of commands to save in
  {input command history}[rodoc-ref:IRB@Input+Command+History];
  initial value: +1000+.
