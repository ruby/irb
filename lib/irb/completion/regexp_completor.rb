    ...
    class RegexpCompletor
      ...
      def complete_require_path(target, preposing, postposing)
        ...
        if tok && tok.event == :on_ident && tok.state == Ripper::EXPR_CMDARG
          case tok.tok
          when 'require'
            result = InputCompletor.retrieve_files_to_require_from_load_path.select { |path|
              path.start_with?(actual_target)
            }.map { |path|
              quote + path
            }
          when 'require_relative'
            result = InputCompletor.retrieve_files_to_require_relative_from_current_dir.select { |path|
              path.start_with?(actual_target)
            }.map { |path|
              quote + path
            }
